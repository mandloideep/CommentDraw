...
class UserControllerTest {
    ...
    void signUp_ShouldReturnSuccess_WhenDataIsValid() throws Exception {
        ...
        mockMvc.perform(post("/user/signup")
                        ...
                        .content("{\"email\":\"test@commentdraw.com\", \"password\":\"Password@123\", \"firstName\":\"John\", \"lastName\":\"Doe\"}"))
    ...
    void login_ShouldReturnAccessTokenAndCookie() throws Exception {
        ...
        mockMvc.perform(post("/user/login")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"test@commentdraw.com\", \"password\":\"pass123\"}"))
                .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser(username = "test@commentdraw.com")
    void getMe_ShouldReturnDashboardData() throws Exception {
        DashboardResponse mockDashboard = DashboardResponse.builder()
                .email("test@commentdraw.com")
                .firstName("John")
                .subscriptionType("GOLD")
                .build();

        when(userService.getCurrentUserFromToken("test@commentdraw.com")).thenReturn(mockDashboard);
